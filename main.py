...
# u_chup_targets[owner_uid] = {chat_id: {target_user_id1, ...}}
u_chup_targets = {}
# u_chup_all[owner_uid] = set(chat_ids)
u_chup_all = {}
...
.bhabhi             Bhabhi radar
.gyan NAME          Unsolicited gyan check
.acting NAME        Overacting meter
.kismat             Luck/Badluck score
.pappu NAME         Masoomiyat scanner
.bhagode            Group lurker check
.system NAME        System hang check
.chugli NAME        Chugli radar report
.fake NAME          Fake raees scanner
.bewakoof NAME      Dimag capacity scan
.bhaukaal NAME      Local don check
.aura               Aura score
.sigma              Sigma score
.sus                Suspicion meter
.evil               Evil meter
...
🎬 HEAVY ANIMATIONS
.gadi               Thar drift system entry
.moye               Moye moye breakdown
.fbi                FBI SWAT raid scene
.thanos             Gauntlet snap & dust
.burn               Burnol dispatch kit
.crying             Anime tears flood
.snitch             Secret camera scan
.fight              Anime boxing K.O.
...
.bill               Fake restaurant bill
.kutai NAME         Digital dhulai machine
.shayari            Funny shayari line
.insult NAME        Savage insult line
.party              Party bill scanner
...
async def start_handler(event):
    uid = event.sender_id
    if event.is_private:
        await bot_users_col.update_one({"user_id": uid}, {"$set": {"user_id": uid}}, upsert=True)
...
async def broadcast_handler(event):
    ...
    status = await event.respond("🚀 **Mega Broadcasting (Users + Groups) shuru ho rahi hai...**")
    
    target_chats = set()

    async for doc in bot_users_col.find({}):
        target_chats.add(doc["user_id"])
    ...
    for chat_id in target_chats:
        try:
            if reply:
                await bot.forward_messages(chat_id, reply)
            else:
    ...
    await status.edit(
        f"✅ **Mega Broadcast Complete!**\n\n"
        f"🎯 Delivered: `{success}`\n"
        f"❌ Failed/Blocked: `{failed}`\n"
        f"📊 Total Target Entities: `{len(target_chats)}`"
    )
